...
void number_obstacles(Mat input_obstacle_map)     //Used to identify individual obstacles and find their edges
{
	...
	for(i=0;i<obstacle_map.rows-1;i++)
	{
		for(j=0;j<obstacle_map.cols-1;j++)
		{
			...
			if(x>0) {
				if(4*((int)sqrt(x*x + y*y))<255) obstacles_numbered.at<uchar>(obstacle_map.rows-1-i,obstacle_map.cols-1-j) = 4*((int)sqrt(x*x + y*y));
				else obstacles_numbered.at<uchar>(obstacle_map.rows-1-i,obstacle_map.cols-1-j);
	...
	//imshow("Output coloured",coloured_obstacles);
	waitKey(10);
	for(int i=0;i<prev_count;i++) {
